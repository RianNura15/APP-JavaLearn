    ...
    <div class="flex pt-16 overflow-hidden bg-gray-50 dark:bg-gray-900">
        ...
        <div id="main-content" class="relative w-full h-full overflow-y-auto bg-gray-50 lg:ml-64 dark:bg-gray-900">
            <main> 
                                ...
                                <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                                    ...
                                    <li>
                                        <div class="flex items-center">
                                            ...
                                            <a :href="dataPeserta" class="ml-1 text-gray-700 hover:text-primary md:ml-2 dark:text-gray-300 dark:hover:text-white">Data Peserta</a>
                                        </div>
                                    </li>
                                </ol>
                            </nav>
                            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Data Peserta</h1>
                        </div>
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                        ...
                                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700" v-for="item in items" :key="item.id">
                                            ...
                                            <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                                ...
                                                <div class="text-sm font-normal text-gray-500 dark:text-gray-400">ID : P-{{ item.id }}</div>
                                            </td>
                                            <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ item.email }}</td>
                                            <td class="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">
                                                <span v-if="item.active == 1" class="bg-green-500 text-sm text-white py-1 px-3 rounded-md">Online</span>
                                                <span v-else class="bg-red-500 text-sm text-white py-1 px-3 rounded-md">Offline</span>
                                            </td>
                                            <td class="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">
                                                <span v-if="item.status == 1" class="bg-green-500 text-sm text-white py-1 px-3 rounded-md">Verified</span>
                                                <span v-else class="bg-red-500 text-sm text-white py-1 px-3 rounded-md">Not Verified</span>
                                            </td>

                                            <td class="p-4 space-x-2 whitespace-nowrap">
                                                <button type="button" id="updateProductButton" :data-drawer-target="`drawer-update-${ item.id }`" :data-drawer-show="`drawer-update-${ item.id }`" :aria-controls="`drawer-update-${ item.id }`" data-drawer-placement="right" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary hover:bg-blue-600 focus:ring-4 focus:ring-blue-700 dark:bg-primary dark:hover:bg-blue-600 dark:focus:ring-blue-700">
                ...
                <!-- Edit Product Drawer -->
                <div v-for="item in items" :key="item.id" :id="`drawer-update-${ item.id }`" class="fixed top-0 right-0 z-[9999] w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
                    <h5 id="drawer-label" class="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">Update Data Peserta</h5>
                    ...
                    <form action="#">
                        <div class="space-y-4">
                            <div>
                                <label for="name" class="block mb-2 mt-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                                <input type="text" name="title" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" :value="item.name" placeholder="Type product name">
                            </div>
                            <div>
                                <label for="name" class="block mb-2 mt-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                                <input type="text" name="title" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" :value="item.email" placeholder="Type product name">
                            </div>
                            <div>
                                <label for="category" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status</label>
                                <select id="category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" v-if="item.status == 1">
                                    <option :value="item.status">Verified</option>
                                    <option value="0">Not Verified</option>
                                </select>
                                <select id="category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" v-else>
                                    <option :value="item.status">Not Verified</option>
                                    <option value="1">Verified</option>
                                </select>
                            </div>
                        </div>
                        <div class="bottom-0 left-0 flex justify-center w-full pb-4 mt-4 space-x-4 sm:absolute sm:px-4 sm:mt-0">
                            <button type="submit" class="w-full justify-center text-white rounded-lg border border-primary hover:bg-primary focus:ring-4 focus:ring-blue-700 dark:border-primary dark:hover:bg-primary dark:focus:ring-blue-700">
                                Update
                            </button>
...
import axios from "axios";
    export default {
        ...
        methods: {
            ...
            async fetchData() {
                try {
                    const response = await axios.get('get-data-peserta');
